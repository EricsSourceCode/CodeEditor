...
public final class MainWindow extends JFrame
  ...
  private void addComponents( Container pane )
    ...
    // Setting it to FixedHeightMax means
    // this component is
    // stretchable.
  ...
  private void resetMainFontBigger()
    {
    mainFontSize++;
    resetFonts();
    }

  private void resetMainFontSmaller()
    {
    mainFontSize--;
    resetFonts();
    }
...
Contol V is the standard paste.
    menuItem.setAccelerator( KeyStroke.
                       getKeyStroke(
                       KeyEvent.VK_P,
                       ActionEvent.CTRL_MASK ));
...
*/
    ...
    menuItem.setBackground( Color.black );
    menuItem.setFont( mainFont );
    menuItem.setActionCommand( "WindowMax" );
    menuItem.addActionListener( this );
    ...
    menuItem.setBackground( Color.black );
    menuItem.setFont( mainFont );
    menuItem.setActionCommand( "WindowNormal" );
    menuItem.addActionListener( this );
    windowMenu.add( menuItem );
    ...
    menuItem.setBackground( Color.black );
    menuItem.setFont( mainFont );
    menuItem.setActionCommand( "WindowMin" );
    menuItem.addActionListener( this );
    windowMenu.add( menuItem );

    menuItem = new JMenuItem( "Font Bigger" );
    menuItem.setMnemonic( KeyEvent.VK_B );
    menuItem.setForeground( Color.white );
    menuItem.setBackground( Color.black );
    menuItem.setFont( mainFont );
    menuItem.setActionCommand( "WindowFontBigger" );
    menuItem.addActionListener( this );
    windowMenu.add( menuItem );

    menuItem = new JMenuItem( "Font Smaller" );
    menuItem.setMnemonic( KeyEvent.VK_S );
    menuItem.setForeground( Color.white );
    menuItem.setBackground( Color.black );
    menuItem.setFont( mainFont );
    menuItem.setActionCommand( "WindowFontSmaller" );
    menuItem.addActionListener( this );
    windowMenu.add( menuItem );
...
*/
    ...
    if( command == "WindowMax" )
      {
      setExtendedState( JFrame.MAXIMIZED_BOTH );
      return;
      }

    if( command == "WindowNormal" )
      {
      setExtendedState( JFrame.NORMAL );
      return;
      }

    if( command == "WindowMin" )
      {
      setExtendedState( JFrame.ICONIFIED );
      return;
      }

    if( command == "WindowFontBigger" )
      {
      resetMainFontBigger();
      return;
      }

    if( command == "WindowFontSmaller" )
      {
      resetMainFontSmaller();
      return;
      }
  ...
  private void runBuildFile()
    ...
    String runFile = mApp.projectConfigFile.
                         getString( "BuildFile" );
  ...
  private void runExecutableFile()
    ...
    if( executableProcess != null )
      {
      showStatus( "The executable process is" +
                         " already running." );
      return;
      }

    String runFile = mApp.projectConfigFile.
                   getString( "ExecutableFile" );
    ...
    // File log = new File(
     // "\\Eric\\CodeAnalysisCpp\\ATestLog.txt" );
  ...
  private void openFile()
    ...
    FilePickDialog fileD = new FilePickDialog(
                                       this,
                                       mApp,
                                       mainFont,
                                       rec.width,
    ...
    String pathName = fileD.getFilePathName();
    String fileName = fileD.getFileName();
    showStatus( "fileName: " + fileName );
    showStatus( "pathName: " + pathName );
...
*/
  ...
  private void keyboardTimerEvent()
    ...
    */
    ...
    ECTime currentTime = new ECTime();

    String showText = "Line: " + line +
          "             " + currentTime.
                  ToLocalTimeStringNoSeconds() +
                  "   " + currentTime.
...
*/
  ...
  private JTextArea getSelectedTextArea()
    ...
    int selectedIndex = mainTabbedPane.
                                  getSelectedIndex();
    if( selectedIndex < 0 )
      return null;
  ...
  private void setCurrentProject()
    ...
    File projFile = new File( pathName );
    ...
    mApp.projectConfigFile.setString(
                       "ProjectDirectory",
                       workingDir, false );
    ...
    setShowProjectText();
      ...
      {
      String showS =
       "Exception with opening project file.\n" +
                     "Entered: " + fileToOpen;
  ...
  private void saveFileAs()
    ...
    if( selectedIndex >= tabPagesArray.length )
      ...
      showStatusTab();
      showStatus(
            "The selected index is past the" +
            " TabPagesArray length." );
      return;
      }
    ...
    if( selectedIndex <= 0 )
      ...
      showStatusTab();
      showStatus(
        "There is no tab page selected, or" +
        " the status page is selected. (Top.)" );
      return;
      }
    ...
    FileSaveDialog fileD = new FileSaveDialog(
                                       this,
                                       mApp,
                                       mainFont,
                                       rec.width,
  ...
  private void showBuildLog()
    ...
    String fileName = mApp.projectConfigFile.
               getString( "ProjectDirectory" );
    fileName += MainApp.pathSepChar + "Build.log";
  ...
  private void showExeOutput()
    ...
    String fileName = mApp.projectConfigFile.
                getString( "ProjectDirectory" );
    fileName += MainApp.pathSepChar + "ExeOut.txt";
  ...
  private void setProjectExecutable()
    ...
    File exeFile = new File( pathName );
    ...
    mApp.projectConfigFile.setString(
            "ExecutableFile", pathName, true );
    showStatus( "Exec File: " +
           mApp.projectConfigFile.getString(
           "ExecutableFile" ));
    }
    catch( Exception e )
      {
      showStatusTab();
      showStatus(
            "Exception with naming exec file." );
      showStatus( e.getMessage() );
      }
    }
  ...
  private void setProjectBuildFile()
    ...
    File buildFile = new File( pathName );

    showStatus( "Path name picked is: " +
                                     pathName );

    mApp.projectConfigFile.setString(
                   "BuildFile", pathName, true );
    showStatus( "Build File: " +
               mApp.projectConfigFile.getString(
                                 "BuildFile" ));
    }
    catch( Exception e )
      ...
      showStatusTab();
      showStatus(
            "Exception with naming Build file." );
      showStatus( e.getMessage() );
      }
    }
  ...
  private void resetFonts()
    ...
    for( int count = 1; count < tabPagesArrayLast;
                                         count++ )
      {
      tabPagesArray[count].getTextArea().
                            setFont( mainFont );
...
====  private Thread fileThread;
    ...
    fileThread.start();
      ...
      {
      mApp.showStatus( "Exception in listFiles()." );
